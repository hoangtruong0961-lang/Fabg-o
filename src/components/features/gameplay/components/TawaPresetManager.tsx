...
export default function TawaPresetManager({
  ...
  const handleSaveAndApply = async () => {
    ...
    try {
      ...
      dbService.setKeyValue(PRESETS_STORAGE_KEY, presets);
      dbService.setKeyValue(ACTIVE_PRESET_ID_KEY, activePresetId);
      ...
      if (activeWorld && activeWorld.activeSaveId) {
        ...
        toast.success("💾 Đã lưu cấu hình Preset vào Nhật Ký Thế Giới thành công!");
      } else {
        toast.success("💾 Đã lưu cấu hình Preset vào trình duyệt thành công!");
      }
    } catch (error) {
      console.error("Failed to save preset to database:", error);
      toast.error("Lỗi khi lưu cấu hình Preset!");
    }
  };
